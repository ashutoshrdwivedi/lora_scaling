...
echo "=== [1/8] sysname Table-4 reference ==="
...
echo "=== [2/8] headline LateFuse sweep (Fig 2 + scaling table) ==="
uv run python -m lora_serving.benchmark.run \
...
echo "=== [7/8] PEFT homogeneous (single-tenant batch) ==="
uv run python -m benchmarks.baselines.peft_swap \
  --model "$M" --dtype fp16 --lora-ranks 8 --seq-len 128 --batch-sizes 8 32 128 \
  --adapters 100 1000 --mode homogeneous --warmup 5 --iters 20 \
