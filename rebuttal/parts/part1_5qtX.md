...
**Only one model tested.** Holding the system fixed, we re-ran the paper's full grid (same protocol, 5 seeds) on three further encoders and a second GPU class: ELECTRA-large (334M, a replaced-token-detection discriminator), DeBERTa-v2-xlarge (885M, disentangled attention), XLM-RoBERTa-XL (3.5B, 36 layers), and bge-m3 on an L40S-48GB. The results table is present in our general response. Three things replicate everywhere:
