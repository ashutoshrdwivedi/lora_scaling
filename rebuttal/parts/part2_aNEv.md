...
We also extend the implementation now with a wrapper code to support different hugging face models to make it genertic for any model. We use it across four encoders spanning three pretraining families, two GPU classes, and a 10× span of model sizes (334M → 3.5B). The serving path is unchanged across all of them. This reproduces the paper's results on each (general-response table).
...
**Future directions.** We plan to release our work as a pip-installable library to serve multi-tenant encoder LoRA models.
