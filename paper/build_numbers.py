...
def build() -> None:
    ...
    (OUT / "table_assembly.tex").write_text(render_table_assembly(reb))
...
def emit_assembler_macros(m: Macros, reg: dict[str, Any]) -> None:
    ...
    # Same-cell tail pairing at B=64 (bge-m3's worst baseline cell). The *Max
    # macros above take their max over B independently, so pairing them would
    # compare 4.29x at B=64 against 1.10x at B=16.
    m.add("AsmBaselineTailAtBSixtyFour", f'{bge["baseline_tail_ratio"]["64"]:.2f}')
    m.add("IdxSelTailAtBSixtyFour", f'{bge["indexsel_tail_ratio"]["64"]:.2f}')
    ...
    # Baseline throughput at the same B as MiniLmIdxSelTputMax (B=256), so the
    # pair divides to MiniLmSpeedupMax. MiniLmBaselineTputMax is at B=128 and
    # must not be paired with it.
    # torch.compile's marginal cost on the gather, across every cell of both
    # models. Reported in the tab:assembly caption instead of a column, which
    # overflowed the ACL text width.
    _comp_delta = [
        mdl[f"indexsel_compile_assemble_ms_by_batch"][str(b)]
        - mdl["indexsel_assemble_ms_by_batch"][str(b)]
        for mdl in (bge, mini) for b in mdl["batch_sizes"]
    ]
    m.add("CompileAsmOverheadMinMs", f"{min(_comp_delta):.2f}")
    m.add("CompileAsmOverheadMaxMs", f"{max(_comp_delta):.2f}")
...
def render_table_assembly(reg: dict[str, Any]) -> str:
    """tab:assembly body: CPU baseline vs. GPU-resident index_select gather.

    Two model blocks, one row per batch size. Latencies are the measured
    per-batch means from rebuttal/numbers.json (not back-derived from the
    rounded throughputs), so the speedup column is reproducible from the
    two latency columns beside it.
    """
    a = reg["assembly"]
    L = ["\\begin{tabular}{@{}rrrrrr@{}}", "\\toprule"]
    L.append("$\\mathcal{B}$ & \\multicolumn{2}{c}{Latency (ms)} & "
             "Speed & Asm.\\ share & p99/p50 \\\\")
    L.append("\\cmidrule(lr){2-3}")
    L.append(" & base & idx-sel & -up & base $\\to$ idx & "
             "base $\\to$ idx \\\\")
    for key, label in (("bgem3", "\\texttt{bge-m3}"),
                       ("minilm", "\\texttt{all-MiniLM-L6-v2}")):
        d = a[key]
        L.append("\\midrule")
        L.append(f"\\multicolumn{{6}}{{@{{}}l}}{{{label}}} \\\\")
        for b in d["batch_sizes"]:
            k = str(b)
            L.append(
                f"{b} & {d['baseline_total_ms_by_batch'][k]:.2f} & "
                f"{d['indexsel_total_ms_by_batch'][k]:.2f} & "
                f"{d['speedup_by_batch'][k]:.2f}$\\times$ & "
                f"{d['baseline_assemble_share_pct'][k]:.1f} $\\to$ "
                f"{d['indexsel_assemble_share_pct'][k]:.1f}\\% & "
                f"{d['baseline_tail_ratio'][k]:.2f} $\\to$ "
                f"{d['indexsel_tail_ratio'][k]:.2f} \\\\")
    L.append("\\bottomrule")
    L.append("\\end{tabular}")
    return "\n".join(L) + "\n"
