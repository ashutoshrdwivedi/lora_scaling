...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/latency_adapters.png}
...
\paragraph{PEFT scales with neither batch size nor adapter count.}
Because the sequential path runs one \texttt{set\_adapter} call plus a
$\mathcal{B}{=}1$ forward per sample, PEFT throughput is invariant in
$\mathcal{B}$ and pinned at $\approx$11.7~samples/sec at $N{=}100$, falling
$8.4\times$ to $\approx$1.4~samples/sec at $N{=}1{,}000$ as the per-swap
path incurs $\mathcal{O}(N)$ overhead. \sysname{}, in contrast, reaches
$\approx$800~samples/sec at $\mathcal{B}{=}64$ with statistically identical
throughput between $N{=}100$ and $N{=}20{,}000$
(Table~\ref{tab:main_results}). Cold start tells the same story: PEFT
takes 1{,}327~s ($\approx$22~minutes) to register 1{,}000 adapters into
its model graph, while \sysname{}'s \texttt{AdapterStore} accepts the same
1{,}000 adapters in under 2~seconds---a $\mathbf{>600\times}$ improvement
in deployment time.
