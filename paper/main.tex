...
samples/sec} with 42~ms p99 latency at batch size 32, and scales from 100 to
...
% that P99 stays bound (P99 itself may be noisy so we need the stable P50 metric as well.)
  statistically unchanged at batch size 32 on a single A100-80GB, with serving
  ...
  improvements of \textbf{37--528$\times$} over the grouped baseline and
...
latency effectively unchanged (40.2 vs.\ 40.3~ms;
...
$N$ & $\mathcal{B}$ & $r$ & p50 & p90 & p99 & QPS & GPU \\
    &     &     & (ms)& (ms)& (ms)& (s/s) & (GB) \\
\midrule
    100 & 32 & 8  & 40.2 & 40.8 & 44.1 & 793 & 1.43 \\
  1{,}000 & 32 & 8  & 40.2 & 40.7 & 45.7 & 779 & 2.87 \\
  5{,}000 & 32 & 8  & 40.2 & 40.5 & 40.9 & 783 & 9.24 \\
 10{,}000 & 32 & 8  & 40.2 & 40.5 & 41.3 & 781 & 17.21 \\
 20{,}000 & 32 & 8  & 40.3 & 40.5 & 42.5 & 779 & 33.15 \\
\midrule
  1{,}000 & 8  & 8  & 21.8 & 22.4 & 23.1 & 364 & 2.77 \\
  1{,}000 & 16 & 8  & 25.7 & 26.2 & 31.3 & 617 & 2.80 \\
  1{,}000 & 64 & 8  & 76.3 & 76.9 & 192.5 & 801 & 3.00 \\
  1{,}000 & 128& 8  & 149.9 & 157.8 & 271.8 & 797 & 3.25 \\
\midrule
  1{,}000 & 32 & 4  & 39.9 & 40.3 & 51.1 & 773 & 2.08 \\
  1{,}000 & 32 & 16 & 39.6 & 39.9 & 41.7 & 806 & 4.44 \\
  1{,}000 & 32 & 32 & 39.7 & 40.0 & 40.3 & 805 & 7.75 \\
\bottomrule
\end{tabular}
\end{table}
...
statistically unchanged (40.2 vs.\ 40.3~ms, a 0.1\% difference)---a
...
assembler whose share remains below 38\% of total latency.
...
Throughput scales from 364~samples/sec at $\mathcal{B}{=}8$ to 801~samples/sec at
$\mathcal{B}{=}64$, then plateaus---$\mathcal{B}{=}128$ delivers no further gain (797~samples/sec).
...
(39.9, 39.7, 39.6, 39.7~ms respectively), with no rank trend in the tail either
(p99 = 51.1, 47.1, 41.7, 40.3~ms). This is the expected behaviour while
...
encoder forward---so the per-batch GPU forward stays at $\approx$26~ms across all
...
tenant ceiling ($\approx$47{,}000 adapters at $r{=}8$ versus $\approx$12{,}000 at
...
per-layer lists, pad the LR head, host-to-device copy) consumes 17\% of iteration time at $\mathcal{B}{=}8$
and \textbf{43\%} at $\mathcal{B}{=}128$, scaling linearly with $\mathcal{B}$
while the encoder forward saturates at $\mathcal{B}{=}64$.
This same path drives tail latency: the p99/p50 ratio is $1.14\times$ at
$\mathcal{B}{=}32$ but reaches $2.52\times$ at $\mathcal{B}{=}64$, with
...
\cite{sheng2024slora}) replacing our \texttt{torch.bmm} shrink/expand help? Three
measurements at $\mathcal{B}{=}32$, $N{=}1{,}000$, $r{=}8$ on an A100-SXM4-80GB
bound the upside to single digits. The \emph{analytic FLOP ratio} per layer is
...
kernels target only this path, so $8.3\%$ is the hard ceiling here. The
decoder-LoRA literature reports larger, multiplicative gains because memory-bound
per-token decode gives LoRA far higher relative weight; the compute-bound encoder
forward at $d{=}1024$ does not benefit comparably.
...
at $\mathcal{B}{=}8/32/128$, $N$-independent). We also report PEFT's deployment
cost---the time to register $N$ adapters, which we replace with a constant-time
...
\emph{Homogeneous} batching is PEFT's best case---single-tenant batches, one
\texttt{set\_adapter} and one forward each---and is competitive at $N{=}100$, even
edging \sysname{} at $\mathcal{B}{=}128$ (1{,}019 vs.\ 828~samples/sec). Yet
holding the batching fixed and growing the pool to 1{,}000 collapses its
throughput 6--9$\times$ (390$\to$48~samples/sec at $\mathcal{B}{=}32$), leaving
\sysname{} 5--31$\times$ faster; since only the adapter \emph{count} changed, this
isolates \texttt{set\_adapter} as $\mathcal{O}(N)$. \emph{Grouped} batching fares
worse still---mixed batches rarely share an adapter, so \sysname{} leads by
54--528$\times$ at $\mathcal{B}{=}32$.
...
enabling mixed-tenant batches without weight merging, custom CUDA kernels, or
KV-cache machinery---serving 20{,}000 adapters on an A100-80GB
(\texttt{BAAI/bge-m3}) at constant latency, a $37$--$528\times$ gain over PEFT
swapping, in standard PyTorch.
