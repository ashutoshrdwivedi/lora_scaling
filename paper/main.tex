...
when samples share an adapter and otherwise degenerating to one
\texttt{set\_adapter(id)} plus a single-sample forward---the default per-request
pattern. It thus issues no more swaps than naive per-request serving, dominating
it by construction. \emph{Base}, the single-tenant ceiling, is the
bare model batched once with no adapters---the upper bound for any per-request
...
\paragraph{Even grouped PEFT collapses under adapter diversity.}
Grouping by adapter does not survive mixed-tenant
load: a batch drawn uniformly from $N \gg \mathcal{B}$ adapters is nearly
all-distinct ($N(1-(1-1/N)^{\mathcal{B}}) \approx \mathcal{B}$), so each bucket
holds $\approx$1 sample and grouping degenerates to per-sample swapping. Its
throughput is therefore $\approx$13--22~samples/sec at $N{=}100$ and collapses
$\approx$10$\times$ to $\approx$1.5 at $N{=}1{,}000$ (Table~\ref{tab:baselines})
as each $\mathcal{O}(N)$ swap dominates---and even the no-adapter base ceiling
caps any per-request scheme at 2{,}094~samples/sec. \sysname{} instead reaches
$\approx$820~samples/sec at $\mathcal{B}{=}128$ with throughput flat from
$N{=}100$ to $20{,}000$ (Table~\ref{tab:main_results})---a $40$--$562\times$
speedup over grouped, plus a $\mathbf{>600\times}$ cold-start gain
($\approx$1{,}200~s to register 1{,}000 adapters vs.\ under 2~s for the
\texttt{AdapterStore}).
...
$40$--$562\times$ throughput over PEFT's adapter-grouped swap baseline.
